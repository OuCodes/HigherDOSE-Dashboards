            ...
            for selector in description_selectors:
                try:
                    ...
                    if desc_element:
                        ...
                        if desc_text and len(desc_text.strip()) > 20:  # Reasonable length for description
                        ...
                        for element in elements:
                            ...
                            if text and len(text.strip()) > 30:  # Substantial description
                                ...
                                if any(skip_phrase in text.lower() for skip_phrase in [
                                    'about', 'posts', 'photos', 'videos', 'events', 'community',
                                    'see all', 'more', 'facebook', 'follow', 'message'
                                ]):
                                    continue
...
async def main_async(argv: Optional[List[str]] = None):
    ...
    parser.add_argument("--data-dir", default="data/facebook", help="Directory containing comment files")
...
def main(argv: Optional[List[str]] = None):
    ...
    try:
        ...
        if sys.platform == "win32":
            ...
            print(f"{ansi.yellow}Note:{ansi.reset} Any asyncio warnings above are just Windows cleanup noise and can be ignored.")
